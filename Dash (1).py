...
@app.callback(
    [Output('output-submit-account', 'children'),
     Output('create-account-window', 'className'),
...
def update_output_submit_account(n_clicks, first_name, last_name, email, new_password):
    if n_clicks:
        ...
        # Set the class name to make the create account window invisible
        create_account_window_class = 'create-account-window-style'

        # Set the number of clicks for the login button to open the login popup
        login_button_n_clicks = 1

        # Return the message indicating successful account creation, the updated class name,
        # and the number of clicks for the login button
        return f"Account created successfully for {first_name} {last_name} with email {email}.", create_account_window_class, login_button_n_clicks

    # If no submission has been made, return initial values
